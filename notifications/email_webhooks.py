...
class EmailWebhookNotifier:
    ...
    async def _send_webhook_notification(self, service_name: str, config: Dict, 
        ...
        try:
            payload = {
                "recipients": self.rebalance_emails,
                "subject": subject,
                "body": body,
                "transaction_data": {
    ...
    async def _send_smtp_notification(self, subject: str, body: str, alert: TransactionAlert) -> bool:
        ...
        try:
            ...
            msg['To'] = ", ".join(self.rebalance_emails)
            msg['Subject'] = subject
        ...
        <body>
            <div class="container">
                ...
                <div class="footer">
                    ...
                    <p>This email was sent to: {", ".join(self.rebalance_emails)}</p>
    ...
    def _generate_daily_summary_content(self, summary: DailySummary) -> tuple[str, str]:
        ...
        <body>
            <div class="container">
                ...
                <div class="footer">
                    <p>🤖 Generated by AI Trading System | {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}</p>
                    <p>This daily summary was sent to: {", ".join(self.daily_summary_emails)}</p>
    ...
    async def _send_daily_webhook_notification(self, service_name: str, config: Dict, 
        ...
        try:
            payload = {
                "recipients": self.daily_summary_emails,
                "subject": subject,
                "body": body,
                "summary_data": {
                    "date": summary.date,
    ...
    async def _send_daily_smtp_notification(self, subject: str, body: str, summary: DailySummary) -> bool:
        ...
        try:
            ...
            msg['Subject'] = subject
            msg['From'] = smtp_config["email"]
            msg['To'] = ", ".join(self.daily_summary_emails)
            ...
            with smtplib.SMTP(smtp_config["smtp_server"], smtp_config["port"]) as server:
                ...
                server.login(smtp_config["email"], smtp_config["password"])
                text = msg.as_string()
                server.sendmail(smtp_config["email"], self.daily_summary_emails, text)
...
async def send_daily_summary_email(portfolio_value: float, cash_balance: float, 
    ...
    # Calculate day change percentage properly
    if portfolio_value > 0 and day_change != 0:
        # Previous portfolio value = current - day_change
        previous_portfolio_value = portfolio_value - day_change
        day_change_percent = (day_change / previous_portfolio_value) * 100 if previous_portfolio_value > 0 else 0.0
    else:
