    ...
    def get_tooled_chain(
        ...
        def token_generator(
            ...
            for token in llm.stream(state, config=config, stop="</function>"):
                if token.response_metadata and "finish_reason" in token.response_metadata:
                    ender = "]</function>"
        ...
        class FakeToolBind(Runnable):
            ...
            def invoke(self, *args, **kwargs): 
                buffer = None
                for token in self.generator(args, **kwargs):
            ...
            async def ainvoke(self, *args, **kwargs): 
                buffer = None
                async for token in gen_to_agen(self.generator(args, **kwargs)):
                    buffer = token if not buffer else buffer + token
                return message_chunk_to_message(buffer)
